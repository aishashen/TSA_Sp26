...
```
Class notes: Annual = frequency will be 1
...
autoplot(ts_inflow_data[,1],series="HP1") + autolayer(ts_inflow_data[,2],series="HP2")+
  xlab("Time") +
  ylab("Inflow") +
  labs(color="Reservoir")
```
...
HP1_pacf=acf((ts_inflow_data[,2],lag.max=40,plot=TRUE))
  
```
Class notes: ACF allows you to better see if there are seasonal trends. Use PCF to eliminate the temporal dependence of the first alphat1. 

ACF & PCF are the first filter to identify if there is a trend at all. They give you regression coefficients.

ACF Plot vs PACF Plot
ACF tells you if the data has memory.

PACF is less intense. Significant lags can be included in PACF linear regression.